...
productRouter.get('/:id', async (req, res) => {
    try{
        const {articulo,precio,imagen} = await productosDao.getById(req.params.id);
        // producto? res.status(200).json(producto) : res.status(404).json({message: 'Producto no encontrado'});
        articulo||precio||imagen? res.status(200).render('detalleProducto',{articulo,precio,imagen}) : res.status(404).json({message: 'Producto no encontrado'});
    }
    catch (err){
        res.status(500).json({message: err.message});
    }
